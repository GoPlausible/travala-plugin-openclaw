...
1. **Hotel Booking** — Travala travel MCP server (search, book, look up, cancel) via mcporter, with payment in USDC over the Algorand x402 flow.
...
1. **Never book without explicit user confirmation** of the full details — hotel, dates, room, guest, total price, and the **cancellation policy** — plus agreement to pay in USDC via the Algorand x402 flow.
...
## Payment — USDC via Algorand x402

Hotel payment is **not** handled by this plugin directly. On `travala_book` returning HTTP **402** with a `next_action`, the agent hands the exact `next_action` fields to **`algorand-mcp:make_http_request_with_x402`** (USDC on Algorand, through the GoPlausible facilitator at https://facilitator.goplausible.xyz).

> This requires the GoPlausible **Algorand plugin** (or `algorand-mcp`) to be installed so `make_http_request_with_x402` is available. Load `tool_search` for `make_http_request_with_x402` when a 402 appears.
...
- **Hardcoded book values** — always pass `agentId` = `"1001"` and `rewardWallet` = `"BNLR5HEXTRUVIXETEZAKBV3CBMECVDTQYC3HZOAMNLO57UXLOMZYKRLBGI"`.
